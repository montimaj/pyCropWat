...
precipitation analysis using real data from the Rio de la Plata basin.
...
6. Effective precipitation method comparison (CROPWAT, FAO/AGLW, Fixed %, Dependable Rainfall)
...
PEFF_METHODS = ['cropwat', 'fao_aglw', 'fixed_percentage', 'dependable_rainfall']
...
def compare_peff_methods():
    ...
    Compares CROPWAT, FAO/AGLW, Fixed Percentage, and Dependable Rainfall
    methods using the same input precipitation data.
    """
    import matplotlib.pyplot as plt
    import numpy as np
...
def compare_methods_for_month(
    ...
    # Find the precipitation file (need to reconstruct from Peff)
    # For this demo, we'll use the existing Peff and show method curves instead
    ...
    # Back-calculate approximate precipitation from CROPWAT Peff
    # This is an approximation for demonstration
    # For P <= 250: Peff = P * (125 - 0.2*P) / 125
    # Solving: P ≈ Peff / (1 - 0.2*Peff/125) for small Peff
    peff_vals = da_cropwat.values.copy()
    
    # Approximate inverse of CROPWAT formula
    ...
    peff_methods = {
        'cropwat': cropwat_effective_precip(precip_approx),
        'fao_aglw': fao_aglw_effective_precip(precip_approx),
        'fixed_percentage': fixed_percentage_effective_precip(precip_approx, 0.7),
        'dependable_rainfall': dependable_rainfall_effective_precip(precip_approx, 0.75)
    }
