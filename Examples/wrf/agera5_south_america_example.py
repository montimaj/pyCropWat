"""
pyCropWat AgERA5 South America Effective Precipitation Example
==============================================================

This script demonstrates an efficient workflow for computing effective
precipitation using AgERA5 data for a South American study region.
...
AgERA5 Precipitation Asset:
    projects/climate-engine-pro/assets/ce-ag-era5-v2/daily
    (daily, band: Precipitation_Flux, mm/day, summed to monthly)
...
AWC and ETo Default Assets (Global):
    ...
    ETo: AgERA5 Daily (projects/climate-engine-pro/assets/ce-ag-era5-v2/daily,
         band: ReferenceET_PenmanMonteith_FAO56, mm/day, summed to monthly)

Study Area:
    Rio de la Plata Basin (projects/ssebop-471916/assets/Riodelaplata)

Output Structure:
    ./AgERA5_SA/usda_scs/                  - USDA-SCS Peff + fraction rasters
    ./AgERA5_SA/analysis_inputs/           - Saved AWC and ETo inputs
    ./AgERA5_SA/peff_by_method/{method}/   - All 8 methods' Peff rasters

Usage:
    python agera5_south_america_example.py
    python agera5_south_america_example.py -w 8
    python agera5_south_america_example.py --gee-project your-project-id
    python agera5_south_america_example.py --download-only
    python agera5_south_america_example.py --calc-only
    python agera5_south_america_example.py -f   # force re-download
"""
...
# AgERA5 precipitation asset (daily, summed to monthly by core)
AGERA5_ASSET = "projects/climate-engine-pro/assets/ce-ag-era5-v2/daily"
AGERA5_BAND = "Precipitation_Flux"
AGERA5_SCALE_FACTOR = 1.0  # AgERA5 precipitation is already in mm/day
...
ETO_SCALE_FACTOR = 1.0  # AgERA5 ETo is already in mm/day
...
def download_usda_scs(n_workers: int = 10, skip_if_exists: bool = True):
    ...
    logger.info("Downloading AgERA5 rasters using USDA-SCS method...")
    logger.info(f"  Precipitation : {AGERA5_ASSET} (band: {AGERA5_BAND})")
    ...
    ep = EffectivePrecipitation(
        asset_id=AGERA5_ASSET,
        precip_band=AGERA5_BAND,
        gee_geometry_asset=STUDY_AREA_ASSET,
        start_year=START_YEAR,
        end_year=END_YEAR,
        precip_scale_factor=AGERA5_SCALE_FACTOR,
...
def calculate_all_methods():
    ...
    if not USDA_SCS_DIR.exists():
        ...
        logger.error("Run download step first (python agera5_south_america_example.py)")
        return
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'AgERA5 effective precipitation for South America. '
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("pyCropWat - AgERA5 South America Effective Precipitation")
    logger.info("=" * 70)
    logger.info(f"  AgERA5 Asset  : {AGERA5_ASSET} (band: {AGERA5_BAND})")
