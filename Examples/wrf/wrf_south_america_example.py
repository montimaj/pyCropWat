"""
pyCropWat WRF South America Effective Precipitation Example
============================================================

This script demonstrates an efficient workflow for computing effective
precipitation using WRF data for a South American study region.
...
WRF Precipitation Asset:
    projects/azhydro/assets/WRF-ET-SA  (monthly, band: b1, 01/2000-12/2021)
...
Output Structure:
    ./WRF_SA/usda_scs/                  - USDA-SCS Peff + fraction rasters
    ./WRF_SA/analysis_inputs/           - Saved AWC and ETo inputs
    ./WRF_SA/peff_by_method/{method}/   - All 8 methods' Peff rasters

Usage:
    python wrf_south_america_example.py
    python wrf_south_america_example.py -w 8
    python wrf_south_america_example.py --gee-project your-project-id
    python wrf_south_america_example.py --download-only
    python wrf_south_america_example.py --calc-only
    python wrf_south_america_example.py -f   # force re-download
"""
...
# WRF precipitation asset
WRF_ASSET = "projects/azhydro/assets/WRF-ET-SA"
WRF_BAND = "b1"
...
def download_usda_scs(n_workers: int = 10, skip_if_exists: bool = True):
    ...
    logger.info("Downloading WRF rasters using USDA-SCS method...")
    logger.info(f"  Precipitation : {WRF_ASSET} (band: {WRF_BAND})")
...
def calculate_all_methods():
    ...
    if not USDA_SCS_DIR.exists():
        ...
        logger.error("Run download step first (python wrf_south_america_example.py)")
        return
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'WRF effective precipitation for South America. '
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("pyCropWat - WRF South America Effective Precipitation")
    logger.info("=" * 70)
    logger.info(f"  WRF Asset     : {WRF_ASSET} (band: {WRF_BAND})")
